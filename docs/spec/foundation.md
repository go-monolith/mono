...
- **Non-goals**: cluster, gateway, leafnode, websocket and MQTT TLS are untouched, and `ServerInfo().ClusterURL` stays `nats://`. HTTPS monitoring is not enabled. Manual certificate files are not supported. A certificate cache shared between clustered nodes is not supported either: `autocert.DirCache` has no distributed locking, so each node needs its own domain, its own cache, and its own reachable port 80.
